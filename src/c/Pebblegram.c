...
static void destroy_unselected_message_images(void) {
  for (int i = 0; i < MAX_MESSAGES; i++) {
...
static void recalc_message_layout(void) {
  ...
  s_chat_content_height = y + (ROUND_UI ? 12 : 3);
  ...
  for (int i = first; i < s_message_count; i++) {
    ...
    if (message->image_placeholder) {
      ...
      if (message->image_bitmap) {
        ...
        int draw_w = PG_MIN(bitmap_bounds.size.w, image_rect.size.w);
        int draw_h = PG_MIN(bitmap_bounds.size.h, image_rect.size.h);
        GRect draw_rect = GRect(image_rect.origin.x + (image_rect.size.w - draw_w) / 2,
                                image_rect.origin.y + (image_rect.size.h - draw_h) / 2,
                                draw_w, draw_h);
...
// Load only the selected message's image. This keeps heap use predictable on
// 64 KB watches and lets images unload naturally as the user scrolls.
static void request_next_image(void) {
  if (s_selected_message < 0 || s_selected_message >= s_message_count) {
    return;
  }
  destroy_unselected_message_images();

  Message *message = &s_messages[s_selected_message];
  if (!message->image_placeholder || !message->image_token[0] ||
...
static void inbox_received_callback(DictionaryIterator *iter, void *context) {
  ...
  if (strcmp(type, "messages_done") == 0) {
    ...
    selected_id[0] = '\0';
    if (s_user_scrolled_messages && s_selected_message >= 0 && s_selected_message < s_message_count) {
    ...
    if (preserved_index >= 0) {
      ...
      s_selected_message = s_message_count > 0 ? (s_loading_older_messages ? 0 : s_message_count - 1) : -1;
    }
    s_loading_older_messages = false;
    s_expected_rows = count;
...
static void action_layer_update_proc(Layer *layer, GContext *ctx) {
  ...
  if (s_action_mode == ActionMenuConfirm) {
    ...
    graphics_draw_text(ctx, s_pending_text, fonts_get_system_font(FONT_KEY_GOTHIC_18),
                       GRect(18, 10, bounds.size.w - 28, 70),
                       GTextOverflowModeWordWrap, GTextAlignmentLeft, NULL);
  ...
  if (s_action_mode == ActionMenuFullText) {
    ...
    graphics_draw_text(ctx, text, full_font,
                       GRect(18, 8 - s_full_text_scroll_offset, text_w, s_full_text_height),
                       GTextOverflowModeWordWrap, GTextAlignmentLeft, NULL);
    return;
  }

  for (int i = 0; i < count; i++) {
    GRect row = GRect(rail_w, top + (i * row_h), bounds.size.w - rail_w, row_h);
    ...
    graphics_draw_text(ctx, action_item_title(i), fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD),
                       GRect(row.origin.x + 8, row.origin.y + 5, row.size.w - 14, row.size.h - 6),
                       GTextOverflowModeTrailingEllipsis, GTextAlignmentLeft, NULL);
  }
}
...
static void action_select_click_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  if (s_action_mode == ActionMenuConfirm) {
    close_action_window();
    if (selected == 0) {
      send_text_message(s_pending_text, true);
    }
  }
}
